        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {testimonials.map((t) => (
            <div key={t.name} className="border border-[#E8E3DA] p-8 transition-colors duration-500 hover:border-[#6B4E31]/40">
              <p className="mb-6 font-light italic leading-relaxed text-[#2F2F2F]" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1rem", lineHeight: 1.9, ...getListItemStyle(content, "testimonials", t.sortOrder, "description", "home") }}>「{t.quote}」</p>
              <p className="text-xs tracking-[0.2em] uppercase text-[#B5956A]" style={getListItemStyle(content, "testimonials", t.sortOrder, "title", "home")}>{t.name}</p>
              <p className="mt-1 text-xs tracking-wide text-[#8C8479]" style={getListItemStyle(content, "testimonials", t.sortOrder, "subtitle", "home")}>{t.info}</p>
            </div>
          ))}
        </div>
      </div>
